    ...
    if( !b_lookahead )
    {
        ...
        if( h->param.b_sliced_threads )
        {
            ...
            if( h == h->thread[0] )
                ...
                h->deblock_strength[0] = h->thread[0]->deblock_strength[0];
        }
        else
...
void x264_macroblock_slice_init( x264_t *h )
{
    ...
    if( h->i_ref[0] > 0 )
    {
        ...
        h->fdec->inv_ref_poc[0] = (256 + delta/2) / delta;
    }
...
void x264_macroblock_bipred_init( x264_t *h )
{
    ...
    {
        x264_frame_t *l0 = h->fref[0][i_ref0>>0];
        int poc0 = l0->i_poc + 0*l0->i_delta_poc[0^(i_ref0&1)];
        for( int i_ref1 = 0; i_ref1 < (h->i_ref[1]<<0); i_ref1++ )
        {
            int dist_scale_factor;
            x264_frame_t *l1 = h->fref[1][i_ref1>>0];
            int cur_poc = h->fdec->i_poc + 0*h->fdec->i_delta_poc[0];
            int poc1 = l1->i_poc + 0*l1->i_delta_poc[0^(i_ref1&1)];
            ...
            h->mb.dist_scale_factor_buf[0][0][i_ref0][i_ref1] = dist_scale_factor;
            ...
            {
                h->mb.bipred_weight_buf[0][0][i_ref0][i_ref1] = 64 - dist_scale_factor;
                ...
                h->mb.bipred_weight_buf[0][0][i_ref0][i_ref1] = 32;
        }
    }
}
