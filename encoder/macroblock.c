...
static void mb_encode_i16x16( x264_t *h, int p, int i_qp )
{
    ...
    if( h->mb.b_trellis )
    {
        for( int idx = 0; idx < 16; idx++ )
            if( x264_quant_4x4_trellis( h, dct4x4[idx], i_quant_cat, i_qp, ctx_cat_plane[DCT_LUMA_AC][p], 1, !!p, idx ) )
            {
                ...
                h->quantf.dequant_4x4( dct4x4[idx], i_qp );
    ...
    if( nz )
    {
        ...
        h->quantf.dequant_4x4_dc( dct_dc4x4, i_qp );  /* XXX not inversed */
        if( block_cbp )
            for( int i = 0; i < 16; i++ )
            ...
            if( h->mb.b_trellis )
            {
                for( int i4x4 = 0; i4x4 < 4; i4x4++ )
                {
                    if( x264_quant_4x4_trellis( h, dct4x4[i8x8*4+i4x4], CQM_4IC+b_inter, i_qp, DCT_CHROMA_AC, !b_inter, 1, 0 ) )
                    {
                        ...
                        h->quantf.dequant_4x4( dct4x4[i8x8*4+i4x4], i_qp );
...
static ALWAYS_INLINE void macroblock_encode_internal( x264_t *h, int plane_count, int chroma )
{
    ...
    else    /* Inter MB */
    {
        ...
        else if( h->mb.b_transform_8x8 )
        {
            ...
            b_decimate &= !h->mb.b_trellis; // 8x8 trellis is inherently optimal decimation for CABAC
            ...
            for( int p = 0; p < plane_count; p++, i_qp = h->mb.i_chroma_qp )
            {
                ...
                if( i_decimate_mb >= 6 || !b_decimate )
                {
                    ...
                    {
                        h->quantf.dequant_8x8( dct8x8[idx], i_qp );
