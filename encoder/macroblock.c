...
void x264_mb_encode_chroma( x264_t *h, int b_inter, int i_qp )
{
        ...
        if( h->pixf.var2[chromapix]( h->mb.pic.p_fenc[1], h->mb.pic.p_fdec[1], ssd ) < thresh*4 )
        {
            ...
            for( int ch = 0; ch < 2; ch++ )
            {
                if( ssd[ch] > thresh )
                {
                    ...
                    {
                        nz_dc = h->quantf.quant_2x2_dc( &dct_dc[0], h->quant4_mf[CQM_4IC+b_inter][i_qp][0] >> 1,
                                                            h->quant4_bias[CQM_4IC+b_inter][i_qp][0] << 1 );
                    }

                    if( nz_dc )
                    {
                        if( !mb_optimize_chroma_dc( h, dct_dc, dequant_mf, i_qp, 0 ) )
                            continue;
    ...
    for( int ch = 0; ch < 2; ch++ )
    {
        ...
        h->dctf.sub8x8_dct( &dct4x4[0], p_src, p_dst );
        ...
        if( h->mb.b_trellis )
        {
            ...
            {
                if( x264_quant_4x4_trellis( h, dct4x4[i4x4], CQM_4IC+b_inter, i_qp, DCT_CHROMA_AC, !b_inter, 1, 0 ) )
                {
                    int idx = 16+ch*16+i4x4;
                    h->zigzagf.scan_4x4( h->dct.luma4x4[idx], dct4x4[i4x4] );
                    h->quantf.dequant_4x4( dct4x4[i4x4], dequant_mf, i_qp );
        ...
        {
            nz = h->quantf.quant_4x4x4( &dct4x4[0], h->quant4_mf[CQM_4IC+b_inter][i_qp],
        ...
        if( i_decimate_score < 7 || !nz_ac )
        {
            ...
            h->dctf.add8x8_idct_dc( p_dst, &dct_dc[0] );
        }
        else
        {
            h->mb.i_cbp_chroma = 1;
            ...
            h->dctf.add8x8_idct( p_dst, &dct4x4[0] );
        }
    }
...
void x264_macroblock_encode( x264_t *h )
{
    ...
    else if( h->mb.i_type == I_8x8 )
    {
        ...
        for( int i = (h->mb.i_skip_intra) ? 3 : 0; i < 4; i++ )
        {
            ...
            x264_mb_encode_i8x8( h, 0, i, i_qp, i_mode, NULL, 1 );
        }
    }
    else if( h->mb.i_type == I_4x4 )
    {
        ...
        for( int i = (h->mb.i_skip_intra) ? 15 : 0; i < 16; i++ )
        {
            ...
            x264_mb_encode_i4x4( h, 0, i, i_qp, i_mode, 1 );
        }
    }
    else    /* Inter MB */
    {
        ...
        else if( h->mb.b_transform_8x8 )
        {
            ...
            {
                nz = x264_quant_8x8( h, dct8x8[idx], i_qp, ctx_cat_plane[DCT_LUMA_8x8][0], 0, 0, idx );
            ...
            if( i_decimate_mb >= 6 || !b_decimate )
            {
                ...
                FOREACH_BIT( idx, 0, plane_cbp )
                {
                    ...
                    h->dctf.add8x8_idct8( &h->mb.pic.p_fdec[0][8*(idx&1) + 8*(idx>>1)*FDEC_STRIDE], dct8x8[idx] );
                    STORE_8x8_NNZ( 0, idx, 1 );
                }
            }
        }
        else
        {
            ...
            for( int i8x8 = 0; i8x8 < 4; i8x8++ )
            {
                ...
                if( h->mb.b_trellis )
                {
                    ...
                    {
                        int idx = i8x8*4+i4x4;
                        if( x264_quant_4x4_trellis( h, dct4x4[idx], quant_cat, i_qp, ctx_cat_plane[DCT_LUMA_4x4][0], 0, 0, idx ) )
                        {
...
int x264_macroblock_probe_skip( x264_t *h, int b_bidir )
{
    ...
    for( int ch = 0; ch < 2; ch++ )
    {
        ...
        if( h->quantf.quant_2x2_dc( &dct_dc[0], h->quant4_mf[CQM_4PC][i_qp][0] >> 1,
        ...
        /* calculate dct coeffs */
        int nz = h->quantf.quant_4x4x4( &dct4x4[0], h->quant4_mf[CQM_4PC][i_qp], h->quant4_bias[CQM_4PC][i_qp] );
        FOREACH_BIT( idx, 0, nz )
        {
            h->zigzagf.scan_4x4( dctscan, dct4x4[idx] );
            int i_decimate_mb = h->quantf.decimate_score15( dctscan );
            if( i_decimate_mb >= 7 )
                return 0;
        }
    }
...
void x264_macroblock_encode_p8x8( x264_t *h, int i8 )
{
    ...
    if( h->mb.b_lossless )
    {
        ...
        for( int ch = 0; ch < 2; ch++ )
        {
            ...
            int offset = i8;
            nz = h->zigzagf.sub_4x4ac( h->dct.luma4x4[16+offset+ch*16], p_fenc, p_fdec, &dc );
    ...
    else
    {
        if( h->mb.b_transform_8x8 )
        {
            ...
            int nnz8x8 = x264_quant_8x8( h, dct8x8, i_qp, ctx_cat_plane[DCT_LUMA_8x8][0], 0, 0, i8 );
            if( nnz8x8 )
            {
                ...
                if( nnz8x8 )
                {
                    ...
                    STORE_8x8_NNZ( 0, i8, 1 );
                    h->mb.i_cbp_luma |= 1 << i8;
                }
                else
                    STORE_8x8_NNZ( 0, i8, 0 );
            }
            else
                STORE_8x8_NNZ( 0, i8, 0 );
        }
        ...
        {
            {
                ...
                if( h->mb.b_trellis )
                {
                    for( int i4x4 = 0; i4x4 < 4; i4x4++ )
                    {
                        if( x264_quant_4x4_trellis( h, dct4x4[i4x4], quant_cat, i_qp, ctx_cat_plane[DCT_LUMA_4x4][0], 0, 0, i8*4+i4x4 ) )
                        {
                            h->zigzagf.scan_4x4( h->dct.luma4x4[i8*4+i4x4], dct4x4[i4x4] );
                            h->quantf.dequant_4x4( dct4x4[i4x4], h->dequant4_mf[quant_cat], i_qp );
                            if( i_decimate_8x8 < 4 )
                                i_decimate_8x8 += h->quantf.decimate_score16( h->dct.luma4x4[i8*4+i4x4] );
                            h->mb.cache.non_zero_count[x264_scan8[i8*4+i4x4]] = 1;
                            nnz8x8 = 1;
                        }
                    }
                }
        ...
        for( int ch = 0; ch < 2; ch++ )
        {
            ...
            h->dctf.sub4x4_dct( dct4x4[0], p_fenc, p_fdec );

            if( h->mb.b_noise_reduction )
                h->quantf.denoise_dct( dct4x4[0], h->nr_residual_sum[2], h->nr_offset[2], 16 );
            dct4x4[0][0] = 0;

            if( h->mb.b_trellis )
                nz = x264_quant_4x4_trellis( h, dct4x4[0], CQM_4PC, i_qp, DCT_CHROMA_AC, 0, 1, 0 );
            else
                nz = h->quantf.quant_4x4( dct4x4[0], h->quant4_mf[CQM_4PC][i_qp], h->quant4_bias[CQM_4PC][i_qp] );

            int offset = i8;
            h->mb.cache.non_zero_count[x264_scan8[16+offset+ch*16]] = nz;
            if( nz )
            {
                h->zigzagf.scan_4x4( h->dct.luma4x4[16+offset+ch*16], dct4x4[0] );
                h->quantf.dequant_4x4( dct4x4[0], h->dequant4_mf[CQM_4PC], i_qp );
                h->dctf.add4x4_idct( p_fdec + 4*0*FDEC_STRIDE, dct4x4[0] );
            }
        }
        h->mb.i_cbp_chroma = 0x02;
    }
...
{
    int i_qp = h->mb.i_qp;
    int quant_cat = 0 ? CQM_4PC : CQM_4PY;
    pixel *p_fenc = &h->mb.pic.p_fenc[0][block_idx_xy_fenc[i8]];
    pixel *p_fdec = &h->mb.pic.p_fdec[0][block_idx_xy_fdec[i8]];
    int nz;
    ...
    if( h->mb.b_lossless )
    {
        nz = h->zigzagf.sub_4x4( h->dct.luma4x4[i8], p_fenc, p_fdec );
        h->mb.cache.non_zero_count[x264_scan8[i8]] = nz;
    }
    else
    {
        ALIGNED_ARRAY_64( dctcoef, dct4x4,[16] );
        h->dctf.sub4x4_dct( dct4x4, p_fenc, p_fdec );
        nz = x264_quant_4x4( h, dct4x4, i_qp, ctx_cat_plane[DCT_LUMA_4x4][0], 0, 0, i8 );
        h->mb.cache.non_zero_count[x264_scan8[i8]] = nz;
        if( nz )
        {
            h->zigzagf.scan_4x4( h->dct.luma4x4[i8], dct4x4 );
